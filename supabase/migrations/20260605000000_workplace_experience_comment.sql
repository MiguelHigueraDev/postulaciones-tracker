-- Optional free-text comment about working at the company (accepted offers).
ALTER TABLE public.workplace_profiles
  ADD COLUMN work_experience_comment text;

ALTER TABLE public.workplace_profiles
  ADD CONSTRAINT workplace_profiles_work_experience_comment_max_length
  CHECK (
    work_experience_comment IS NULL
    OR char_length(work_experience_comment) <= 1000
  );

DROP FUNCTION IF EXISTS public.submit_feedback(
  text, text, text, text, text, int, text, text, text,
  int, text, text, text, text, smallint, smallint, smallint, smallint
);
...
CREATE OR REPLACE FUNCTION public.submit_feedback(
  ...
  p_work_experience_comment text DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_has_profile := p_salary IS NOT NULL
    ...
    OR p_work_experience_comment IS NOT NULL;

  IF v_has_profile AND p_result = 'Oferta - Aceptada' THEN
    INSERT INTO workplace_profiles (
      submission_id, salary, good_things, bad_things, benefits, modality,
...
GRANT EXECUTE ON FUNCTION public.submit_feedback(
  text, text, text, text, text, int, text, text, text,
  int, text, text, text, text, smallint, smallint, smallint, smallint, text
